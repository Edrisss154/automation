...
import { saveLetterTemplate } from '../../../api/api';

const AddHeaderModal = ({ isOpen, toggle }) => {
    ...
    const elements = [
        { id: 'signerName', label: 'نام امضا کننده' },
        { id: 'letterContent', label: 'متن نامه' },
        { id: 'subject', label: 'موضوع سند/نامه' },
        { id: 'internalLetterNumber', label: 'شماره نامه داخلی' },
        { id: 'externalLetterNumber', label: 'شماره نامه ص/و' },
        { id: 'createdDate', label: 'تاریخ ایجاد' },
        { id: 'letterSignature', label: 'امضای نامه' },
        { id: 'signerPosition', label: 'سمت امضا کننده' },
        { id: 'postalCode', label: 'کد پستی مجموعه' },
        { id: 'contactNumber', label: 'شماره تماس مجموعه' },
        { id: 'documentImage', label: 'تصویر سند دبیرخانه' },
        { id: 'issueDate', label: 'تاریخ صادره/وارده' },
        { id: 'carbonCopy', label: 'رونوشت' },
        { id: 'attachment', label: 'پیوست' },
        { id: 'logo', label: 'لوگو مجموعه' },
        { id: 'address', label: 'آدرس مجموعه' },
        { id: 'faxNumber', label: 'شماره فکس' },
        { id: 'emailWebsite', label: 'آدرس ایمیل و سایت' },
        { id: 'sender', label: 'فرستنده' },
        { id: 'recipient', label: 'گیرنده' }
    ];
    ...
    const handleElementClick = (element) => {
        if (!addedElements.has(element.id)) {
            setDroppedElements([...droppedElements, { ...element, x: 50, y: 50, width: 120, height: 40 }]);
            setAddedElements(new Set([...addedElements, element.id]));
        }
    };
    ...
    const handleRemoveElement = (id) => {
        ...
        setAddedElements(new Set([...addedElements].filter(elId => elId !== id)));
    };
    ...
    const handleSubmit = async () => {
        ...
        if (backgroundImage) {
            formData.append('background', backgroundImage, backgroundImage.name);
        }
        ...
        try {
            const response = await saveLetterTemplate(formData);
        ...
        <Modal isOpen={isOpen} toggle={toggle} size="lg" className="rtl">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl">
                ...
                <div className="bg-blue-900 dark:bg-blue-800 px-6 py-4 flex justify-between items-center rounded-t-lg">
                    <h3 className="text-xl font-bold text-white">اضافه کردن سربرگ جدید</h3>
                ...
                <div className="p-6 overflow-x-auto">
                    ...
                    <div className="w-full mb-6">
                        ...
                        {/* Elements Container - Now responsive */}
                        <div className="flex flex-wrap gap-2 mt-6 max-w-3xl mx-auto">
                            {elements.map((element) => (
                                <button
                                    key={element.id}
                                    onClick={() => handleElementClick(element)}
                                    disabled={addedElements.has(element.id)}
