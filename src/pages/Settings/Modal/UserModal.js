...
import { addContact, getRoles } from '../../../api/api';

const UserModal = ({ isOpen, toggle, userData, handleSave }) => {
    ...
    const handleSaveClick = async () => {
        ...
        formPayload.append('is_contact', 0);
        ...
        try {
            const response = await addContact(formPayload);
            console.log("Response from server:", response.data);
            handleSave();
            toggle();
        } catch (err) {
            console.error("Error from server:", err.response?.data || err.message);
            alert("خطا در ثبت کاربر. لطفاً دوباره تلاش کنید.");
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto">
            <div className="flex items-center justify-center min-h-screen p-4">
                ...
                {/* Modal */}
                <div className="relative bg-gray-0 dark:bg-gray-900 rounded-lg shadow-xl max-w-2xl w-full mx-4 my-8">
                    {/* Header */}
                    <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-800">
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                            افزودن کاربر یا سمت جدید
                        </h3>
                        <button
                            onClick={toggle}
                    ...
                    <div className="p-6 space-y-6 bg-white dark:bg-gray-900">
                        <div className="space-y-4">
                            <div>
                                <label htmlFor="mobile" className="block text-sm font-bold text-gray-900 dark:text-black-100 mb-2">
                                    شماره موبایل
                                </label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label htmlFor="firstName" className="block text-sm font-bold text-black dark:text-gray-100 mb-2">
                                    نام
                                </label>
                                <input
                                    type="text"
